...
Captured from the running app (see `docs/screenshots/` — 58 shots covering every page, both themes, desktop + mobile, the radial menu, the auth/404 system screens, the session-22 error-surface parity set, the dev-server verification sets, the session-24 layered-behavior set, the session-28 head-metadata + mobile-typography set, the session-30 DOM-texture + cadence set, and the session-32 reduced-motion + motion-profile set):
...
| 🧪 | Vitest unit suites (validation, hashing, JSON parsing, typewriter, constellation incl. the reduced-motion `slotVisible` fallback, menu geometry, db-path resolution, site-config DOM parity incl. the marquee title-case + LOGO_BREATH cadence contracts) + Playwright E2E (7 spec files, 62 tests incl. the 5 opt-in outage specs) | `tests/`, `e2e/` |
...
E2E_ADMIN_PASSWORD=… bunx playwright test       # all e2e (62 tests: 57 normal + 5 outage-skipped; server on :3000)
