...
Captured from the running app (see `docs/screenshots/` — 50 shots covering every page, both themes, desktop + mobile, the radial menu, the auth/404 system screens, the session-22 error-surface parity set, the dev-server verification sets, the session-24 layered-behavior set, and the session-28 head-metadata + mobile-typography set):
...
| `49-dev-legal-h2-text-xl.png` — legal h2 at 20px/28px, no md step (session 28) | `50-dev-head-metadata-surface.png` — the social/PWA head surface (og:image/og:url/canonical/manifest, session 28) | |
...
| 🧪 | Vitest unit suites (validation, hashing, JSON parsing, typewriter, constellation, menu geometry, db-path resolution, site-config DOM parity) + Playwright E2E (7 spec files, 55 tests incl. the 5 opt-in outage specs) | `tests/`, `e2e/` |
...
```bash
bun run test                                   # all unit suites (76 tests)
...
E2E_ADMIN_PASSWORD=… bunx playwright test       # all e2e (55 tests: 50 normal + 5 outage-skipped; server on :3000)
