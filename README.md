...
Captured from the running app (see `docs/screenshots/` — 55 shots covering every page, both themes, desktop + mobile, the radial menu, the auth/404 system screens, the session-22 error-surface parity set, the dev-server verification sets, the session-24 layered-behavior set, the session-28 head-metadata + mobile-typography set, and the session-30 DOM-texture + cadence set):
...
| `55-dev-radial-menu-40px-links.png` — menu links at the source's 40px line-height + "Toggle projects" expander (session 30) | | |
...
| 🧪 | Vitest unit suites (validation, hashing, JSON parsing, typewriter, constellation, menu geometry, db-path resolution, site-config DOM parity incl. the marquee title-case + LOGO_BREATH cadence contracts) + Playwright E2E (7 spec files, 60 tests incl. the 5 opt-in outage specs) | `tests/`, `e2e/` |
...
```bash
bun run test                                   # all unit suites (79 tests)
...
E2E_ADMIN_PASSWORD=… bunx playwright test       # all e2e (60 tests: 55 normal + 5 outage-skipped; server on :3000)
