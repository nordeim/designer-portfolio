...
E2E_ADMIN_PASSWORD=… bunx playwright test    # full e2e (66 tests: 61 normal + 5 outage-skipped; server must be running on :3000)
...
- Reduced-motion is a designed divergence (session 32): the source's JS machines (typewriter, constellation, LOGO_BREATH) all IGNORE `prefers-reduced-motion` (only its CSS animations get the `1e-05s` minimize). We honor it — machines pause — but NEVER at the cost of content: the hero renders a static slot-0 constellation image (`slotVisible` in `src/lib/constellation.ts`) with frozen dots and static typewriter lines. The /projects cursor preview is keyed per row (`key={hovered}`) so row-switches play the source's ~250 ms crossfade (exit scale 0.95 + enter overlap) — a constant key swaps the image silently.
- Document-surface pins (session 34): no `::selection` rule (the source uses the browser default — never re-add a brand tint); the 4px sage webkit-scrollbar rules live in `globals.css` (LightningCSS serializes `background: transparent` as `0px 0px` — a build-serialization divergence, rendered value identical); `button, [role="button"] { cursor: pointer }` is restored in the base layer (Tailwind v4 dropped the v3 preflight rule the source carries); the legal eyebrow is a SPAN storing title-case "Legal" (CSS uppercases); the footer's bottom row ships a hidden CTA anchor as its first child (inert — link-graph parity); the body carries no `font-feature-settings` (the source renders stock Inter; our old "ss01"/"cv11" line was proven inert — the committed gstatic subset lacks those glyphs).
