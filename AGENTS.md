...
| `bunx playwright test` | E2E suite (`e2e/*.spec.ts`, 55 tests: 50 normal + 5 outage specs that skip unless `E2E_OUTAGE=1`) against an already-running server on :3000 |
...
- **Next.js replaces nested metadata objects wholesale** (session 28): a page that sets `openGraph` LOSES the root's siteName/locale/type/images, and the root's `twitter` block SURVIVES unless the page sets its own — so every route-level OG/twitter block is composed through `pageMetadata()` in `src/lib/og.ts`. The og:image is this site's own `/icon.svg` (never a hotlink to the reference CDN), and `public/manifest.json` + the layout's `manifest`/`appleWebApp` metadata keys form the PWA surface. `twitter:url` (which the reference ships) is not expressible in Next's typed API and is ignored by X's parser — documented divergence.
