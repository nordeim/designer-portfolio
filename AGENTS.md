...
| `bunx playwright test` | E2E suite (`e2e/*.spec.ts`, 62 tests: 57 normal + 5 outage specs that skip unless `E2E_OUTAGE=1`) against an already-running server on :3000 |
...
4. The full gate is verified green on a **fresh clone** (`bun install` → `migrate deploy` → `seed`): lint, typecheck, 85 unit tests, coverage 100%, build, 57 E2E tests (+ the 5-spec outage suite under `E2E_OUTAGE=1`). Keep it that way — `tsc --noEmit` must pass with only the dependencies declared in `package.json` (no stale `node_modules` phantom packages).
