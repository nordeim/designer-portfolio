...
| `bun run test` | Vitest unit suites (`tests/*.test.ts`, 85 tests) |
| `bunx vitest run --coverage` | Unit suites + the 100% coverage gate on the pure seam (PAD §7.3) |
| `bunx playwright test` | E2E suite (`e2e/*.spec.ts`, 66 tests: 61 normal + 5 outage specs that skip unless `E2E_OUTAGE=1`) against an already-running server on :3000 |
...
4. The full gate is verified green on a **fresh clone** (`bun install` → `migrate deploy` → `seed`): lint, typecheck, 85 unit tests, coverage 100%, build, 61 E2E tests (+ the 5-spec outage suite under `E2E_OUTAGE=1`). Keep it that way — `tsc --noEmit` must pass with only the dependencies declared in `package.json` (no stale `node_modules` phantom packages).
...
- E2E parity specs pin source-verified metrics (login-card geometry, 404 stack, project-not-found text, legal section anatomy, the error-surface set, the robots/sitemap machine-surface contracts, the works-row hover smoothness, the session-30 DOM-texture + cadence set: marquee title-case storage, copyright single text node, numbering node texture, LOGO_BREATH expanded hold, radial-menu link line-height, the session-32 motion set: the reduced-motion static constellation fallback + the /projects preview row-switch crossfade, and the session-34 document-surface set: browser-default ::selection, the 4px sage webkit-scrollbar, pointer cursors on all buttons, the legal eyebrow's title-case span, the footer's hidden CTA anchor) — changes that regress reference parity fail the suite.
...
- **Playwright + live deployments**: read-only specs run verbatim against any origin via `E2E_BASE_URL=https://… bunx playwright test` (password-gated specs skip) — the post-deploy smoke test documented in `docs/DEPLOYMENT.md`. Remote-origin runs get a 90 s `navigationTimeout` automatically (session 30). Always clear stale ports before `E2E_START` runs (a leftover server from a previous round makes the whole suite fail spuriously with ECONNREFUSED). Under origin congestion, computed-style specs can flake in full-suite runs while passing in isolation — re-run failures individually before diagnosing code.
...
- **Display h1s keep `max-md:leading-tight`** (session 28): the reference's `leading-tight` only WINS below 768px (v3 emits responsive text-size variants after base utilities, so their bundled line-heights win at md+ — 45px mobile, 72/60px desktop). Dropping the class (session 26) broke mobile to 40px; applying it unprefixed would break desktop. Same family: the detail intro h2 carries `leading-snug` (41.25px, unprefixed — no responsive variant there) and the legal h2s are plain `text-xl` (no md step).
- **DOM text-node texture is a parity surface** (session 30): the source's footer copyright is ONE text node, its case-study hero label carries the static "/06 — " as ONE node (3-node label), while its LANDING works label is the 5-node JSX split — match each surface exactly. JSX `text{expr}text{expr}` interleaving splits static text into separate DOM nodes; a template-literal expression renders one. The same session: the marquee's DOM text is title-case (CSS uppercases visually — the session-14 "uppercase as-is" pin was a mis-read), and the breathing logo's cadence rests EXPANDED 3.4 s / tight 3.3 s (`LOGO_BREATH` in site-config — the old machine held expanded for 0.5 s and doubled the tight phase).
- **Radial-menu links follow the v3-cascade line-height rule** (session 30, third occurrence of the session-28 lesson): the source's `leading-tight` wins only below md (37.5 px at 30 px text) while `md:text-4xl`'s bundled 40 px wins at md+ — replicate with `max-md:leading-tight`, never unprefixed (45 px at md = the divergence the spec pins).
- **Reduced-motion is a designed divergence, not a gap** (session 32): the source's `prefers-reduced-motion` handling is a CSS-only half-measure (animations minimized to `1e-05s`) — its JS machines (typewriter, constellation cycling, LOGO_BREATH) all IGNORE the preference and keep running. We deliberately honor it (a11y-positive): machines pause, but the hero keeps its imagery via a STATIC slot-0 fallback (`slotVisible` in `src/lib/constellation.ts`) + frozen dots + static typewriter lines — never let reduced-motion REMOVE content. The /projects cursor preview is keyed per row (`key={hovered}`) so a row-switch plays the source's ~250 ms exit/enter crossfade — a constant key would silently swap the image instantly.
- **Document-surface parity pins** (session 34): the source ships NO `::selection` rule (browser default — never re-add a brand-tinted selection), a global 4px sage webkit scrollbar (`::-webkit-scrollbar*` in `globals.css` — LightningCSS serializes `background: transparent` as `0px 0px`; assert the rule structure, not the keyword), the Tailwind v3-era `button, [role="button"] { cursor: pointer }` rule (v4 dropped it — every source button computes pointer; ours restores it), the legal eyebrow as a SPAN storing title-case "Legal" (`font-mono text-xs tracking-widest uppercase text-muted-foreground block mb-6` — the marquee-class DOM texture, NOT a `<p>` storing "LEGAL"), and a HIDDEN "Start a Project →" anchor as the footer bottom row's first child (`hidden` class — inert, never tabbable, but part of the source's DOM; removing it breaks the 4-anchor /contact link-count parity). The body's `font-feature-settings: "ss01","cv11"` line was REMOVED — it was proven inert (the committed gstatic Inter subset lacks those alternate glyphs; DOM glyph screenshots are byte-identical with or without it), and the source renders stock (`normal`).
