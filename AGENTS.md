...
| `bun run test` | Vitest unit suites (`tests/*.test.ts`, 79 tests) |
| `bunx vitest run --coverage` | Unit suites + the 100% coverage gate on the pure seam (PAD §7.3) |
| `bunx playwright test` | E2E suite (`e2e/*.spec.ts`, 60 tests: 55 normal + 5 outage specs that skip unless `E2E_OUTAGE=1`) against an already-running server on :3000 |
...
4. The full gate is verified green on a **fresh clone** (`bun install` → `migrate deploy` → `seed`): lint, typecheck, 79 unit tests, coverage 100%, build, 55 E2E tests (+ the 5-spec outage suite under `E2E_OUTAGE=1`). Keep it that way — `tsc --noEmit` must pass with only the dependencies declared in `package.json` (no stale `node_modules` phantom packages).
